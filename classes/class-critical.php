...
class workforce_Critical {
	...
	public function enqueue_scripts() {
		...
		if ( ! array_key_exists( 'csscached', $_COOKIE ) ) { return; }
	...
	public function print_header_scripts() {
		...
		if ( array_key_exists( 'csscached', $_COOKIE ) && $_COOKIE['csscached'] === PARENT_THEME_VERSION ) { return; }
		...
		foreach ( $this->stylesheets as $name => $stylesheet ) {
			...
			$newname = workforce_camelcase( $name, '-' );
	...
	public function set_stylesheets() {
		...
		$this->stylesheets['workforce-style'] = get_stylesheet_uri();
		
		/**
		 * The workforce_async_stylesheets filter.
		 */
		$this->stylesheets = apply_filters( 'workforce_async_stylesheets', $this->stylesheets );
	...
	public function set_templates() {
		...
		/**
		 * The workforce_critical_css_templates filter.
		 */
		$this->templates = apply_filters( 'workforce_critical_css_templates', $this->templates );
	...
	public function tweak_tag( $tag, $handle, $href, $media ) {
		...
		if ( 'workforce-style' !== $handle ) { return $tag; }
