...
class workforce_Sanitize {
	...
	public function clean( $data, $type ) {
		...
		if ( empty( $type ) ) {
			...
			$check = new WP_Error( 'forgot_type', __( 'Specify the data type to sanitize.', 'workforce' ) );

		}

		if ( is_wp_error( $check ) ) {

			wp_die( $check->get_error_message(), __( 'Forgot data type', 'workforce' ) );
		 ...
		 */
		do_action( 'workforce_pre_sanitize', $sanitized );
		 ...
		 */
		do_action( 'workforce_post_sanitize', $sanitized );
